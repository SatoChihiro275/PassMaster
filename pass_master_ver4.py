...
if os.path.exists(MASTER_PASSWORD_FILE):
    ...
    # 初回起動時にマスターパスワードを設定
    layout = [
        [sg.Text("初回設定: マスターパスワードを入力してください")],
        [sg.InputText(password_char="*", key="MASTER_PASSWORD")],
        [sg.InputText(password_char="*", key="CONFIRM_PASSWORD")],
        [sg.Button("保存")]
    ]
    window = sg.Window("マスターパスワード設定", layout)
    while True:
        event, values = window.read()
        if event == sg.WINDOW_CLOSED:
            exit()
        if event == "保存":
            if values["MASTER_PASSWORD"] == values["CONFIRM_PASSWORD"]:
                saved_password_hash = hash_password(values["MASTER_PASSWORD"])
                with open(MASTER_PASSWORD_FILE, "w") as file:
                    file.write(saved_password_hash)
                sg.popup("マスターパスワードが保存されました")
                break
            else:
                sg.popup("パスワードが一致しません。もう一度入力してください。")
    window.close()

# 認証のためのマスターパスワード入力
layout = [
    [sg.Text("マスターパスワードを入力してください")],
    [sg.InputText(password_char="*", key="MASTER_PASSWORD")],
    [sg.Button("ログイン")]
]
window = sg.Window("ユーザー認証", layout)
while True:
    event, values = window.read()
    if event == sg.WINDOW_CLOSED:
        exit()
    if event == "ログイン":
        input_password_hash = hash_password(values["MASTER_PASSWORD"])
        if input_password_hash == saved_password_hash:
            sg.popup("ログイン成功")
            break
        else:
            sg.popup("パスワードが間違っています。再試行してください。")
window.close()
